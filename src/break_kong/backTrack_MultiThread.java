...
public class backTrack_MultiThread {
	...
	synchronized public static void write(String s) {
			...
			try {
				...
				fileWriter = new FileWriter("/data/szj/kong/break/11_later_Join.txt", true);
//				fileWriter = new FileWriter("G:\\kong\\break\\11_later_single.txt", true);
	...
	public static class gosub implements Runnable {
		...
		public void run() {
			...
			backTrack_MultiThread.backTrackCombination(this.a, this.m, this.number, this.target, this.allposition);
		}
	...
	public static void main(String[] args) {
		...
		backTrackCombination(arrays,11,1,38178792,allpositions_init);
		while(true) {
			...
			System.out.println("One cycle detected !");
		}
	}
	...
	public static int backTrackCombination(int[] a,int m,int number,int target,int allposition[]) {
		...
		for(int shift =0;shift<=maxshift;shift++) {
			System.out.println(number+"------>"+shift);
...
//			System.out.println(sum);
			sum=sum/11;//取平均数
			if(Math.abs(sum-target)<=0.99999) {
				System.out.println("------------------>");
				String out="";
				for(int j=1;j<=m;j++) {
					out=out+"|"+a[allposition_[j]];
				}
				backTrack_MultiThread.write(out+"\t"+Math.abs(sum-target)+"\n");//输出11个数的组合
				return(0);
			}
			if(sum<target) {//再次向下移动时则总和必定小于目标值，故不再移动
				return(0);
			}
			
			//最小值最小组合大于target则最左端的直接下一个
			int sum_min=0;
			int shift_local=0;
			for(int j=1;j<=m;j++) {
				if(j<number) {
			...
			//current位的基础上的右边下一位进行下一步递归
//			if(number==1&&backTrack_MultiThread.threads_addition>0) {
			if(number<=1) {
				backTrack_MultiThread.threads_addition--;
